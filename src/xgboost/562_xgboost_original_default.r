...
EstimarGanancia_xgboost  <- function( x )
{
  ...
  param_basicos  <- list( gamma=                0.0,  #por ahora, lo dejo fijo, equivalente a  min_gain_to_split
                          alpha=                1.0,  # 0 por ahora, lo dejo fijo, equivalente a  lambda_l1
                          ...
                          max_leaves=           0,    # 0 ya lo voy a cambiar
  ...
  modelocv  <- xgb.cv( objective= "binary:logistic",
                       ...
                       nrounds= 100,        # 9999 | un numero muy grande, lo limita early_stopping_rounds
                       early_stopping_rounds= 20, # nada x default | as.integer(50 + 5/x$eta)
                       base_score= 0.5, #mean( getinfo(dtrain, "label"))
