...
hs  <- makeParamSet(
          makeIntegerParam("num.trees" ,        lower=  100L, upper= 1000L),  #la letra L al final significa ENTERO
          makeIntegerParam("max.depth",         lower=    1L, upper=   30L),  # 0 significa profundidad infinita
          makeIntegerParam("min.node.size" ,    lower=    1L, upper=  20000L),
          makeIntegerParam("mtry" ,             lower=    2L, upper=   20L))


ksemilla_azar  <- 999931  #Aqui poner la propia semilla
...
setwd("C:/Users/usuario/Desktop/CURSOS_VARIOS/2_Maestría_Datos_UTN/2 AÑO/2-AMD-EcoFin/labo/exp\\HT4330\\")
